...
local cfname = "MainDisasterBefore"..http:GenerateGUID(true)
runserv:BindToRenderStep(cfname,Enum.RenderPriority.Camera.Value-1,function()
    if not UI or not UI.Parent then
    runserv:UnbindFromRenderStep(cfname)
    return
    end
...
local rsname = "MainDisaster"..http:GenerateGUID(true)
runserv:BindToRenderStep(rsname,Enum.RenderPriority.Camera.Value+1,function(dt)
    if not UI or not UI.Parent then
    runserv:UnbindFromRenderStep(rsname)
    return
    end
...
local function updateCollisions() end
	...
	local hb = runserv.Heartbeat:Connect(function()
		...
		for _,humMod:Model in ipairs(humanoidModels) do
			...
			for root in pairs(allRoots) do
				...
				local lastNetworkOwner:Instance?,wasNetworkAuto:boolean = nil,true
			...
			task.defer(function()
				...
				ev = hum.StateChanged:Connect(function(_,new)
					...
					for _,data in ipairs(setOwners) do
						local networkOwner,isNetworkAuto = nil,true
