...
function ai.giveAI(rig,type)
if rig == nil then return end
if type == "Cleetus" then
-- Cleetus AI
figure = rig
figureroot = figure.HumanoidRootPart
figurehum = figure.Humanoid
...
coroutine.wrap(function()
	wait(17)
	figurehum.WalkSpeed = 0
	figurehum.JumpPower = 0
	for i,v in next, workspace:GetDescendants() do
	if v:IsA("Model") and v ~= flambro.Value then
	if v:FindFirstChild("DED") then
	v.DED:Destroy()
	end
	end
	end
		...
		if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" then
			spawn(function()
				repeat
					...
					figure.Head.face.Transparency = figure.Head.face.Transparency + 0.05
				until v.Transparency > 0.99
				figure:Destroy()
			end)
		end
	end
...
while true do
	...
	for i,v in pairs(workspace:GetDescendants()) do
		...
		if v == flambro.Value then return end
		if v:FindFirstChild("IsRaider") then return end
		if v:FindFirstChild("LETMEGETYA") then return end
		if v:FindFirstChild("DED") then return end
			...
			if vhum.Health > 0.1 then
				if vroot == nil then
					...
					repeat
						...
						path:ComputeAsync(figureroot.Position,vroot.Position)
						...
						for _,points in pairs(waypoints) do
							...
							if points.Action == Enum.PathWaypointAction.Jump then
								figurehum:ChangeState(Enum.HumanoidStateType.Jumping)
							end
							figurehum:MoveTo(points.Position)
							figurehum.MoveToFinished:Wait()
						end
					until v:FindFirstChild("DED") ~= nil
				end
			end
		end
	end
